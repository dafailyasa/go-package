...
func NewZeroLogHook(appName string) *Logger {
	z := zerolog.New(os.Stdout).Hook(TracingHook{}).With().Str("appName", appName).Timestamp().Stack().Logger()

	return &Logger{log.New(z, "", 0), &z}
}

// NewZeroLog: Creates a new logger with tracing information extracted from the provided context.
func NewZeroLog(ctx context.Context, appName string, c ...io.Writer) *Logger {
	span := trace.SpanContextFromContext(ctx)
